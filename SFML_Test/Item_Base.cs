    ...
    class Item_Base : Sprite
    {
        ...
        public void DeployItem()
        {
            if (isInstantDeploy)
            {
                ...
                if (Phys_Const.rand.Next(0, 100) < 50)
                {
                    vVelocity.X = (float)(Phys_Const.rand.Next(50, 100) / 100f * -1f);
                    isMovingLeft = true;
                }
                else
                {
                    vVelocity.X = (float)(Phys_Const.rand.Next(50, 100) / 100f);
                    isMovingLeft = false;
                }

                vVelocity.Y = (float)(Phys_Const.rand.Next(100, 350) / 100f) * -1f;
